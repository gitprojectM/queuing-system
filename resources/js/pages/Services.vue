    ...
    <div class="w-full max-w-4xl bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8">
      <div class="flex flex-col gap-4 mb-6">
        <div class="flex justify-between items-center">
          ...
          <Link href="/services/create" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-all duration-200">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
            <tr v-for="service in services" :key="service.id" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
              <td class="py-3 px-6">{{ service.name }}</td>
              <td class="py-3 px-6">{{ service.description }}</td>
              <td class="py-3 px-6">
                <Link v-if="service && typeof service.id !== 'undefined' && service.id !== null" :href="`/services/${service.id}/edit`" class="inline-flex items-center gap-1 px-3 py-1.5 rounded-md bg-blue-500 hover:bg-blue-600 text-white text-xs font-medium shadow mr-2 transition">
                ...
                <button
                  v-if="service && typeof service.id !== 'undefined' && service.id !== null"
                  @click.prevent="deleteService(service.id)"
                  class="inline-flex items-center gap-1 px-3 py-1.5 rounded-md bg-red-500 hover:bg-red-600 text-white text-xs font-medium shadow transition"
                >
